...
**Input (all optional):**
- `limit` (integer) - Maximum results to return (default: 100, max: 1000)
...
Imports data from other formats to OASF agent record format.

**Input:**
- `source_data` (string, **required**) - JSON string of the source data to import
- `source_format` (string, **required**) - Source format: `mcp`, `a2a`, or `agentskills` (also accepts `agent-skill`)

**Supported formats:**
- **`mcp`** — MCP server data, wrapped as `{"server": {...}}` or as a raw server object (auto-wrapped)
- **`a2a`** — A2A card as a direct object or wrapped as `{"a2aCard": {...}}`
- **`agentskills`** — Agent Skills markdown wrapped as `{"skillMarkdown": "---\nname: ...\n---\n\n..."}`
...
Exports an OASF agent record to other formats.

**Input:**
- `record_json` (string, **required**) - JSON string of the OASF agent record to export
- `target_format` (string, **required**) - Target format: `a2a`, `ghcopilot`, or `agentskills` (also accepts `agent-skill`)

**Output:**
- `exported_data` (string) - The exported data in the target format (JSON for `a2a` and `ghcopilot`; plain-text SKILL.md markdown for `agentskills`)
...
- `source_format` (string, **required**) - Source format: `mcp`, `a2a`, or `agentskills`
...
**Use when:** You want to import MCP servers, A2A cards, or Agent Skills markdown into the OASF format. This handles all the complexity automatically.
...
- `target_format` (string, **required**) - Target format: `a2a`, `ghcopilot`, or `agentskills`
...
**Use when:** You want to export OASF records to A2A cards, GitHub Copilot MCP configurations, or Agent Skills SKILL.md markdown.
...
- `DIRECTORY_CLIENT_AUTH_MODE` - Authentication mode: `none`, `insecure`, `x509`, `jwt`, `token`, `tls`, `oidc`, or empty for auto-detect (default: empty)
- `DIRECTORY_CLIENT_SPIFFE_SOCKET_PATH` - SPIFFE Workload API socket path (required for `x509` and `jwt` modes)
- `DIRECTORY_CLIENT_SPIFFE_TOKEN` - Path to SPIFFE token file (required for `token` mode)
- `DIRECTORY_CLIENT_JWT_AUDIENCE` - JWT audience (required for `jwt` mode)
- `DIRECTORY_CLIENT_TLS_CERT_FILE` - Client TLS certificate file (required for `tls` mode)
- `DIRECTORY_CLIENT_TLS_KEY_FILE` - Client TLS private key file (required for `tls` mode)
- `DIRECTORY_CLIENT_TLS_CA_FILE` - CA certificate file (required for `tls` mode)
- `DIRECTORY_CLIENT_OIDC_ISSUER` - OIDC issuer URL (for `oidc` mode or auto-detect)
- `DIRECTORY_CLIENT_OIDC_CLIENT_ID` - OIDC client ID (for interactive login via `dirctl auth login`)
- `DIRECTORY_CLIENT_AUTH_TOKEN` - Pre-issued bearer token (for `oidc` mode; skips interactive login)
- `DIRECTORY_CLIENT_TLS_SKIP_VERIFY` - Skip TLS verification (set to `true` if needed)

#### OIDC Authentication

For remote Directory gateways (including GitHub-backed deployments), authentication uses **OIDC** via the Directory client.

**Recommended: Pre-issued bearer token**

For MCP servers running in your IDE, use a long-lived bearer token (for example, a personal access token or CI-issued JWT) via `DIRECTORY_CLIENT_AUTH_TOKEN`. This avoids frequent re-login and IDE restarts.

**Setup steps:**

1. Obtain a bearer token from your Directory administrator or identity provider
2. Set `DIRECTORY_CLIENT_AUTH_MODE` to `"oidc"`
3. Set `DIRECTORY_CLIENT_AUTH_TOKEN` to your token in the MCP configuration (see example below)

**Alternative: Interactive login**

Use `dirctl auth login` to authenticate through your OIDC provider. Cached tokens are stored under `~/.config/dirctl/tokens/` (issuer-scoped). Tokens typically expire after a few hours; re-authenticate and restart your IDE when the token expires.

```bash
dirctl auth login --oidc-issuer=<issuer-url> --oidc-client-id=dirctl
```
...
**Example - Remote gateway with bearer token (Cursor):**
...
**Example - Remote gateway with interactive OIDC login (Cursor):**
...
Before starting the MCP server with interactive OIDC configuration, authenticate using `dirctl auth login` (with the issuer and client ID for your environment). The server loads the cached token from `~/.config/dirctl/tokens/`. Re-authenticate and restart your IDE when the token expires.
...
- "Import this Agent Skills markdown to OASF format: [JSON]"
- "Export this OASF record to A2A format: [JSON]"
- "Export this OASF record to Agent Skills markdown: [JSON]"
...
- `/dir-mcp-server/import_record` - Import from MCP, A2A, or Agent Skills with enrichment
- `/dir-mcp-server/export_record` - Export OASF to A2A, GitHub Copilot, or Agent Skills
