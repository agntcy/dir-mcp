---
name: dirctl-auth
description: Authenticate with the AGNTCY Directory instance configured in dir-mcp using dirctl. Uses the bundled binary or the path set in DIRECTORY_DIRCTL_PATH — never scans PATH. Use when the user asks to log in to the Directory, authenticate with dirctl, run `dirctl auth login`, get or refresh a token, or when MCP tools return auth/permission errors and the auth mode is `oidc`.
---
